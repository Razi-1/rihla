...
export default function ClassInvite() {
  ...
  useEffect(() => {
    inviteService.list().then((res) => setInvites(res.data.data)).finally(() => setLoading(false));
  }, []);
            ...
            {invites.map((inv) => (
              <motion.div key={inv.id} className={styles.card} variants={staggerItem}>
                <div className={styles.cardTop}>
                  <div>
                    <h3>{inv.session.title}</h3>
                    <p className={styles.meta}>
                      <Clock size={14} strokeWidth={1.5} /> {formatDateTime(inv.session.start_time)}
                      <span>&middot;</span>
                      {SESSION_TYPES[inv.session.session_type as keyof typeof SESSION_TYPES]}
                    </p>
                    <p className={styles.tutor}>from {inv.session.tutor_name}</p>
                  </div>
