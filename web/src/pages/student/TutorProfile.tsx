...
export default function TutorProfile() {
  ...
  if (loading) {
    return <PageTransition><div className={styles.page}><Skeleton width="100%" height={300} borderRadius="var(--radius-md)" /></div></PageTransition>;
  }
        ...
        <div className={styles.layout}>
          <div className={styles.main}>
            <div className={styles.profileCard}>
              ...
              {tutor.subjects.length > 0 && (
                <div className={styles.section}><h2>Subjects</h2><div className={styles.chips}>{tutor.subjects.map((s) => <Chip key={s.id}>{s.subject_name} — {s.education_level_name}</Chip>)}</div></div>
              )}
            </div>
                  ...
                  {reviews.map((r) => (
                    <div key={r.id} className={styles.reviewCard}>
                      <div className={styles.reviewHeader}>
                        <Avatar src={r.student_profile_picture} firstName={r.student_name.split(' ')[0] ?? ''} lastName={r.student_name.split(' ')[1] ?? ''} size="sm" />
                        <div><span className={styles.reviewerName}>{r.student_name}</span></div>
          ...
          <aside className={styles.aside}>
            <div className={styles.pricingCard}>
              <h3>Pricing</h3>
              {tutor.individual_rate != null && <div className={styles.priceRow}><DollarSign size={16} strokeWidth={1.5} /><span>Individual: {formatCurrency(tutor.individual_rate, tutor.currency ?? 'USD')}/hr</span></div>}
              {tutor.group_rate != null && <div className={styles.priceRow}><DollarSign size={16} strokeWidth={1.5} /><span>Group: {formatCurrency(tutor.group_rate, tutor.currency ?? 'USD')}/hr</span></div>}
              <Button fullWidth size="lg" onClick={handleChat} loading={chatLoading} icon={<MessageCircle size={18} strokeWidth={1.5} />}>
                Contact Tutor
              </Button>
            </div>
